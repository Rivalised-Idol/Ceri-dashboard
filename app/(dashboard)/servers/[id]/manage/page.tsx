...
// import { useEffect, useState } from 'react';
// import { useParams, useRouter } from 'next/navigation';
// import { fetchServerById } from '@/lib/servers/fetchServerById';
// import { updateServer } from '@/lib/servers/updateServer';
// import { deleteServer } from '@/lib/servers/deleteServer';
...
//     setServer((prev) => (prev ? { ...prev, [field]: value } : prev));
//   };
...
//       {['name', 'ip', 'country', 'city'].map((field) => (
//         <div key={field}>
...
//             onChange={(e) => handleInputChange(field as keyof Server, e.target.value)}
//             className="w-full bg-slate-800 text-white px-3 py-2 rounded-md border border-slate-700"
//           />
//         </div>
//       ))}
...
//         <button
//           onClick={() => setServer({ ...(originalServer as Server) })}
//           disabled={saving || deleting}
//           className={`px-4 py-2 rounded-md font-medium ${
//             saving || deleting
//               ? 'bg-gray-600 text-gray-300 cursor-not-allowed'
...
import { fetchServerById } from "@/lib/servers/fetchServerById";
import type { Server } from "@/types/servers";
import ManageServerClient from "@/components/servers/ManageServerClient";
...
export default async function ManageServerPage({ params }: Props) {
  const { id } = await params; // ✅ Next 15 dynamic params are a Promise
  const numericId = Number(id);
  if (Number.isNaN(numericId)) return notFound();

  let server: Server | null = null;
  try {
    server = await fetchServerById(numericId);
  } catch {
  ...
  return (
    <div className="max-w-lg mx-auto mt-10">
      <ManageServerClient initialServer={server} />
    </div>
  );
}
